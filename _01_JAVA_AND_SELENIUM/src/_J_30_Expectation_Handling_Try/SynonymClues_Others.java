...
. and will be displayed on the pane.Synonym clues will be given to the user to guess the word  .
. and type it in Textfield. Here the encrypted wordmaster file will be decrypted(Using Rijndal .
...
public class SynonymClues extends JPanel implements ActionListener
{
	private JLabel lbl_jumbword,lbl_synclues,lbl_typecorword,lbl_title,lbl_subtitle,lbl_inst,lbl_instexp;
	private JLabel lbl_jumbwordans[],lbl_syncluesans[],lbl_anssym[];	// labels
	...
	private Font f1,f2;
	private StringTokenizer stz_str2,stz_str3,stz_st;	// different string tokenizer variables
	private String[] str_word;
	private String[] str_synonym;
	private String str_sy,str_s;
	public Vector vect_syncluesans,vect_jumbword,vect_jumbwordans;		// vector objs
    ...
    public SynonymClues()
	{
		...
		* and also initializes the different vectors.
		*/
		try{
		...
		lbl_syncluesans = new JLabel[10];
		for(int i=0;i<10;i++)
		{
			lbl_jumbwordans[i] = new JLabel();
			lbl_syncluesans[i] = new JLabel();
		}
		vect_syncluesans = new Vector();
		rand = new Rand();
		vect_jumbwordans = Anagrams.getWordsData(Anagrams.str_filedata[0]);	// it returns vector which holds the answer values
        Synonym();			// calling Synonym method
		jumbword = new JumbledWord();
		vect_jumbword = jumbword.getJumbleWords(vect_jumbwordans);	// it returns vector which holds the jumbled words

		} catch(Exception ANAGRAMS11) { error(ANAGRAMS11,"ANAGRAMS1011"); }

	}   // end of SynonymClues constructor
	...
	public void design()
	{
	    ...
	    */
        ...
        lbl_synclues = new JLabel("Synonym Clues");
		lbl_synclues.setBounds(250,130,200,24);
		add(lbl_synclues);
		lbl_synclues.setFont(new Font("SansSerif",Font.BOLD,14));
		...
		for(int i=0;i<10;i++)
		{
			lbl_anssym[i] = new JLabel();				// creating objs of the different label symbols
	    ...
	    } catch(Exception ANAGRAMS12) { error(ANAGRAMS12,"ANAGRAMS1012"); }
	 ...
	 */
		...
		if(source == btn_back)
		{
		 // if the user pressed back button then it decrements the index val by 10 and shows the previous page
		    clear();		// calling clear the method, which clears the different component values
			btn_showans.addActionListener(this);	
			btn_verify.addActionListener(this);  
			for(int j=0; j<10; j++)
			{
	 	...
	 	}	// end btn_back if condition

		else if(source == btn_showans)
	    {
		int loc_ans=indx_show;
		  /**
		  ...
		  */
			...
			for (int i=0; i<10; i++)
			{
				lbl_anssym[i].setVisible(false);	// setting label answer symbol visibility to false
				tf_ans[i].setText(dec.decryptString((String)vect_jumbwordans.elementAt(loc_ans-1)));
				tf_ans[i].setEditable(false);		// setting text filed answer editable to false
				loc_ans++;			// incrementing record index of the show block
			}	// end of for loop
			repaint();
			flag_verify = false;
			flag = false;
		 ...
		 */
		 ...
		 loc_verify=indx_verify;
			for(int i=0; i<10; i++)
			{
				...
				loc_verify++;			// incrementing the index of verify block
			}	// end of for loop
		...
		else if(source == btn_nextset)
		{
		   ...
		   */
			...
			Anagrams.angcards.show(getParent(),"gotoanagrams");	// showing the card
...
//		     wordmine.main.WordMine1.cards.show(wordmine.main.WordMine1.c,"WordMine");//added for integration
	    ...
	    } catch(Exception ANAGRAMS13) { error(ANAGRAMS13,"ANAGRAMS1013"); }
	 ...
	 */
		...
		if(ind < vect_jumbword.size())
		{
          for(int h=0; h<10; h++)
		  {
			...
			if(dec.decryptString((String)vect_syncluesans.elementAt(ind-1)).equals("null"))
			    lbl_syncluesans[h].setText("--");
			else
			    lbl_syncluesans[h].setText(dec.decryptString((String)vect_syncluesans.elementAt(ind-1))) ;
			lbl_jumbwordans[h].setBounds(70,180+30*h,120,24);
  		    add(lbl_jumbwordans[h]);
		    lbl_syncluesans[h].setBounds(250,180+30*h,225,24);
		    add(lbl_syncluesans[h]);
			ind++;
		 }	// end of for loop
		}	// end of if
	    } catch(Exception ANAGRAMS14) { error(ANAGRAMS14,"ANAGRAMS1014"); }
	...
	public void Synonym()
	{
	  ...
	  /**
	  * Synonym method will read the particular record from readfile and make it into 
	  *	tokens. And stores synonym clues values into particular vector.
	  */
		
		 try{
       	 Tokenizer stz_str2 = new Tokenizer((Anagrams.str_filedata[1]),"#");
		 while(stz_str2.hasMoreElements())
		 {
			Tokenizer stz_str3 = new Tokenizer(stz_str2.nextToken(),"$");
			String  str_s1 = stz_str3.nextToken();
			String[] str_a3 = new String[4];
			str_a3[0] = stz_str3.nextToken();
  		    str_a3[1] = stz_str3.nextToken();
			str_a3[2] = stz_str3.nextToken();
			str_a3[3] = stz_str3.nextToken();
			int ans = Integer.parseInt(dec.decryptString(stz_str3.nextToken()));
			if(ans==0)
				vect_syncluesans.addElement(str_a3[0]);		// adding synclues into vector
			else
				vect_syncluesans.addElement(str_a3[ans-1]);		// adding synclues into vector
        }	// end of while loop
	    } catch(Exception ANAGRAMS15) { error(ANAGRAMS15,"ANAGRAMS1015"); }
	...
	public void clear()
	{
	  ...
	  */
		...
		for(int k=0; k<10; k++)
		{
			...
			lbl_syncluesans[k].setText("");
			tf_ans[k].setText("");
			tf_ans[k].setEditable(true);
			repaint();
		}	// end of for loop
	    } catch(Exception ANAGRAMS16) { error(ANAGRAMS16,"ANAGRAMS1016"); }
	}	// end of clear method
